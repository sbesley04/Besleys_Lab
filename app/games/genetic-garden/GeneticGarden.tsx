    ...
    <div className={styles.stack}>
      ...
      <div className={styles.layout}>
        ...
        <aside className={styles.sidebar}>
          ...
          <div className={`${styles.panel} ${styles.contracts}`}><p className={styles.kicker}>Commissions</p><p className={styles.help}>Only new crosses count — the starter stock just supplies the genes.</p>{CONTRACTS.map((contract) => <div key={contract.id} className={`${styles.contract} ${garden.completed.includes(contract.id) ? styles.contractDone : ""}`}><strong>{garden.completed.includes(contract.id) ? "✓ " : contract.tier === 2 ? "◆ " : ""}{contract.title}</strong><div>{contract.description}</div></div>)}</div>
...
function PlantCard({ plant, selected, assayed, onClick }: { plant: Plant; selected: boolean; assayed: boolean; onClick: () => void }) {
  ...
  // Height 0.75–2.15 maps to an 18–68px stem, and the head rides on top of it
  // (it used to sit at a fixed spot while only the stem scaled, so a tall
  // plant's stem ran up through its own flower).
  const stem = Math.round(18 + ((p.height - 0.75) / 1.4) * 50);
  return <button className={`${styles.plant} ${selected ? styles.plantSelected : ""}`} onClick={onClick} aria-pressed={selected}><span className={styles.flower} aria-hidden="true" style={{ "--stem": `${stem}px` } as CSSProperties}><span className={styles.stem} /><span className={`${styles.flowerHead} ${styles[p.color]}`}>{Array.from({ length: p.petals }, (_, index) => <span key={index} className={styles.petal} style={{ "--petal-angle": `${(360 * index) / p.petals}deg` } as CSSProperties} />)}<span className={styles.flowerCore} /></span></span><span className={styles.plantName}>{plant.name} · F{plant.generation}</span><span className={styles.traits}>{p.color} · {p.petals} petals<br />height {p.height.toFixed(2)} · {p.drought ? "drought hardy" : "needs water"}<br />{assayed ? genotypeLabel(plant) : "genotype unassayed"}</span></button>;
}
