...
export default function BayesianDetective() {
  ...
  return (
    <div className={styles.stack}>
      ...
      {outcome ? <div className={styles.banner} role={outcome.correct ? "status" : "alert"}><h2>{outcome.correct ? "Case closed." : "The accusation does not hold."}</h2><p className={styles.help}>{outcome.correct ? `Correct, with ${budget} inquiry points left and ${(beliefs[outcome.suspect] * 100).toFixed(1)}% posterior confidence.` : `The culprit was ${current.suspects.find((s) => s.id === current.culprit)!.name}. Review which clues actually distinguished the alternatives.`}</p><div className={styles.row} style={{ marginTop: "0.6rem" }}>{caseIndex < CASES.length - 1 ? <button className={`${styles.button} ${styles.primary}`} onClick={() => loadCase(caseIndex + 1)}>Next case →</button> : null}<button className={styles.button} onClick={() => loadCase(caseIndex)}>Reopen this case</button></div></div> : null}
